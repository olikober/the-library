...
Always use the interview tool if installed.
...
Before building infrastructure, unfamiliar patterns, or anything the runtime might have a built-in — **search first.** Read `ETHOS.md` for the full philosophy.
...
Log eureka moments using mulch tool and skill.
...
3. If it does NOT — add a the review report template:
...
4. If existing, use mulch skill to get existing project knowledge
5. **List existing design docs for this project:**
   ```bash
   ls -t ./plans/*-design-*.md 2>/dev/null
   ```
...
```bash
grep -li "<keyword1>\|<keyword2>\|<keyword3>" ./plans/*-design-*.md 2>/dev/null
```
...
This enables cross-team discovery — multiple users exploring the same project will see each other's design docs in `./plans/`.
...
(`/gstack:plan-design-review`, `/gstack:design-review`) to see what was originally envisioned.
...
Write the design document to the project directory: `./plans/{user}-{branch}-design-{datetime}.md`:
...
3. Print and append metrics to the plan

| Iterations | Issues Found | Fixed | Remaining | Score |
|------------|--------------|-------|-----------|-------|
| <ITERATIONS> | <FOUND> | <FIXED> | <REMAINING> | <SCORE> |
...
- **`/gstack:plan-ceo-review`** for ambitious features (EXPANSION mode) — rethink the problem, find the 10-star product
- **`/gstack:plan-eng-review`** for well-scoped implementation planning — lock in architecture, tests, edge cases
- **`/gstack:plan-design-review`** for visual/UX design review

The design doc at `./plans/` is automatically discoverable by downstream skills — they will read it during their pre-review system audit.
